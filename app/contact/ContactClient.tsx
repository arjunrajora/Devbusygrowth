...
export default function ContactClient() {
  ...
  const validateField = (fieldName: keyof FormDataState, value: string): string => {
    ...
    switch (fieldName) {
      ...
      case "interest":
        if (!cleanValue) return "Please select service required.";
        return "";

      case "message":
        if (!cleanValue) return "Project description is required.";
        if (cleanValue.length < 10) return "Please provide at least 10 characters detailing your project.";
        return "";
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    if (touchedFields[name]) {
      const errorMsg = validateField(name as keyof FormDataState, value);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSubmitError("Network error. Please check your internet connection or contact us on WhatsApp.");
    } finally {
